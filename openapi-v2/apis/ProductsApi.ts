...
export class ProductsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param id 
     */
    public async reviseAProduct(body: ReviseProductPayload, id: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'body' is not null or undefined
        if (body === null || body === undefined) {
            throw new RequiredError("ProductsApi", "reviseAProduct", "body");
        }


        // verify required parameter 'id' is not null or undefined
        if (id === null || id === undefined) {
            throw new RequiredError("ProductsApi", "reviseAProduct", "id");
        }
        ...
        requestContext.setHeaderParam("Content-Type", contentType);
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(body, "ReviseProductPayload", ""),
            contentType
        );
        requestContext.setBody(serializedBody);
...
export class ProductsApiResponseProcessor {
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async reviseAProductWithHttpInfo(response: ResponseContext): Promise<HttpInfo<APIProduct >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: APIProduct = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
